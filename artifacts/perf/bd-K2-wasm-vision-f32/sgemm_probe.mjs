// LEVER-1 precondition: is matrixmultiply's wasm simd128 sgemm microkernel live
// in the +simd128 build? Same crate, same shapes, two modules — the ratio answers.
...
const A = load(`${HERE}/wbench-nosimd.wasm`);   // scalar fallback kernel
const B = load(`${HERE}/wbench-simd.wasm`);     // +simd128
...
for (const [label, [m, k, n], rounds] of CASES) {
  ...
  console.log(
    `${label.padEnd(34)} nosimd ${g(best.A).toFixed(2).padStart(6)} GF/s  simd128 ${g(best.B).toFixed(2).padStart(6)} GF/s  ${(best.A / best.B).toFixed(2)}x  checksum ${sums.A === sums.B ? "=" : "DIFFER"}`,
  );
}
