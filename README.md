...
| `FOCR_STAGE_BUDGET_FORWARD_MS` | Override the forward stage budget in milliseconds (default 600000, i.e. 10 minutes). `0` (or `unlimited`) disables the stage timeout entirely — the run is then bounded only by cooperative cancellation (Ctrl+C). |
| `FOCR_NO_RESIDENT` | `1` or `true` disables the resident warm-model daemon (the flag analog is `--no-resident`). By default, eligible single-image `focr ocr` runs are served by a per-model background process that keeps the loaded weights in RAM between invocations. |
| `FOCR_RESIDENT_IDLE_SECS` | How long the resident daemon stays alive with no requests before unloading the model and exiting (default 600 seconds). |
| `FOCR_RESIDENT_DIR` | Override the directory holding resident daemon state files (default: the model cache root). |
| `FOCR_RESIDENT_LOG` | Append resident daemon diagnostics to this file (its stderr is discarded by default). |
| `FOCR_RESIDENT_SPAWN_WAIT_SECS` | How long a client waits for a freshly spawned daemon to accept (default 30). |
| `FOCR_RESIDENT_CLIENT_TIMEOUT_SECS` | Client-side reply timeout for a resident request (default 600). |
...
**Can it run as a server so the model loads once (like llama.cpp)?** Yes — this is automatic. Eligible single-image `focr ocr` runs are served through a **resident warm-model daemon**: the first invocation spawns a per-model background process (loopback-TCP, authenticated by a user-only state file) that keeps the loaded weights in RAM, and every later invocation within the idle window (default 10 minutes, `FOCR_RESIDENT_IDLE_SECS`) skips the multi-gigabyte artifact load. The daemon exits by itself after the idle period, re-verifies the artifact identity on every request, and any daemon problem silently falls back to the classic in-process load — the output contract is identical. Opt out with `--no-resident` or `FOCR_NO_RESIDENT=1`. There is no HTTP API; for programmatic long-lived use, embed the library (`OcrEngine` caches the model for the process lifetime).
