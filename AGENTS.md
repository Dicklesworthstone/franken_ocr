...
Input: **document images** (PNG/JPG/…) **and PDFs**. PDF pages are rasterized **natively, in-process**, in pure memory-safe Rust (`src/pdf.rs`, no FFI). The bounded scan path includes JPEG, CCITT, raw Flate/LZW, pure-Rust JPX/JPEG 2000, JBIG2, and the admitted layered-MRC form (opaque base plus same-page soft-masked overlays). A distinct zero-image path renders the exact vector/path and embedded TrueType glyph dialect emitted by MTDT's score-PDF writer plus the bounded Type1C dialect emitted by LilyPond. Selectable-text extraction decodes MTDT's Type0/Identity-H plus complete ToUnicode representation and LilyPond's Type1C/WinAnsi + Differences representation; every source code and glyph name is retained, and music glyphs without Unicode remain explicitly opaque. This is not general born-digital PDF support: arbitrary vector/text operators, partial mosaics, nested Form XObjects, unsupported transforms, and unsupported blend/composition forms refuse with the exact capability that must be added inside `franken_ocr::pdf`; runtime guidance must never route them through an external rasterizer. See plan §7.7. No Python, no FFI, **no network at inference time**, no GPU required.
